...
class _CartScreenState extends ConsumerState<CartScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: true,
        title: const Text("Your Cart", style: TextStyle(color: Color(0xFF1A1A1A), fontSize: 22, fontWeight: FontWeight.bold)),
      ),
      body: Column(
        children: [
          Expanded(
            child: cartItems.isEmpty
                ? Center(child: Text("Your cart is empty. Go add some cakes!", style: TextStyle(color: Colors.grey.shade500, fontSize: 16)))
                : ListView.builder(
  ...
  Widget _buildCartItem(CartItem item) {
    return Container(
      ...
      child: Row(
        children: [
          Container(height: 70, width: 70, decoration: BoxDecoration(color: primaryBrown.withOpacity(0.1), borderRadius: BorderRadius.circular(12)), child: Icon(Icons.cake, color: primaryBrown.withOpacity(0.5))),
  ...
  Widget _buildReceiptRow(String title, String amount, {bool isTotal = false}) {
    return Row(
      ...
      children: [
        Text(title, style: TextStyle(color: isTotal ? const Color(0xFF1A1A1A) : Colors.grey.shade600, fontSize: isTotal ? 18 : 14, fontWeight: isTotal ? FontWeight.bold : FontWeight.w500)),
        Text(amount, style: TextStyle(color: isTotal ? primaryBrown : const Color(0xFF1A1A1A), fontSize: isTotal ? 20 : 14, fontWeight: isTotal ? FontWeight.bold : FontWeight.w600)),
      ],
    );
  }
}